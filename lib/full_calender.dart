...
class FullCalender {
  ...
  int get valueOfDate => _dateToInt(date.year, date.month, date.day);

  LunarDateTime get lunarDate => _convertSolarDateToLunarDate();
  ...
  int _dateToInt(int y, int m, int d) {
    m = (m + 9) % 12;
    y = y - (m / 10).floor();
    return 365 * y +
        (y / 4).floor() -
  ...
  LunarDateTime _convertSolarDateToLunarDate() {
    int index = date.year - _solar_1_1[0];
    int data = (date.year << 9) | (date.month << 5) | (date.day);
    int solar11 = 0;
    ...
    int offset =
        _dateToInt(date.year, date.month, date.day) - _dateToInt(y, m, d);
